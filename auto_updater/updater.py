...
SNIPPETS_DIR = os.path.join(ROOT, "data", "snippets")
TARGET_DIRS = [os.path.join(ROOT, p) for p in [
    "trailbinder","trailbinder/core","trailbinder/graph","trailbinder/exporters",
    "trailbinder/plugins","trailbinder/utils","trailbinder/tasks","trailbinder/bookmarks"]]
...
def load_snips():
    m={}
    if not os.path.isdir(SNIPPETS_DIR): return m
    for fn in os.listdir(SNIPPETS_DIR):
        p=os.path.join(SNIPPETS_DIR,fn)
